...
<head>
   ...
   <title>Product Archive Section</title>
...
<h1 class="heading">Archived Products</h1>
   ...
   <div class="boxs">
      ...
      <div class="export-button">
         <a href="product_archive.php?export_csv=1" class="btn" style="background-color: #28a745;">Export to CSV</a>
      </div>
   </div>
...
if (isset($_GET['export_csv'])) {
   ...
   header('Content-Disposition: attachment; filename=archived_products.csv');
   $output = fopen("php://output", "w");

   fputcsv($output, array('ID', 'Name', 'Category', 'Price', 'Stock', 'Date Deleted'));

   $export_query = $conn->prepare("SELECT id, name, category, price, stock, date_deleted FROM `product_archive` ORDER BY date_deleted DESC");
   $export_query->execute();

   while ($row = $export_query->fetch(PDO::FETCH_ASSOC)) {
       $row['name'] = strip_tags($row['name']);
       $row['category'] = strip_tags($row['category']);
       $row['price'] = number_format($row['price'], 2); 
       $row['stock'] = (int) $row['stock'];  
       
       $row['date_deleted'] = date("Y-m-d H:i:s", strtotime($row['date_deleted']));
